...
RSpec.describe 'api/users/{user_id}/followings', type: :request do
  path '/api/users/{user_id}/followings' do
    get 'Show user\'s subscriptions' do
      tags 'Profile'
      ...
      let!(:relationship) { create(:relationship, follower_id: user.id, followed_id: user2.id) }
      let(:user_id) { user.id }
